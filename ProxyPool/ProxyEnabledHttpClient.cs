...
namespace ProxyPool
{
    ...
    public class ProxyEnabledHttpClient : IDisposable
    {
            ...
            string userAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36")
        {
            ...
            //   _userAgent = userAgent ?? "Mozilla/5.0";
        ...
        private async Task<string> TryWithNewProxiesAsync(string url, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    //    List<string> randomizedList = proxyAddresses.OrderBy(item => random.Next()).ToList();
            ...
            int maxTimeSeconds)
        {
          ...
          //  var shuffledProxies = proxyAddresses.OrderBy(_ => Guid.NewGuid()).ToList();
        ...
        int maxProxyTestTimeSeconds = 120)
        {
            ...
            _proxyClient = new ProxyEnabledHttpClient(
                    ...
                    testTimeoutSeconds: 3,          // Faster timeout for testing
                    fetchTimeoutSeconds: 20,
                    maxParallelTests: 100,          // Higher parallelism for faster testing
        ...
        public async Task<bool> InitializeProxyPoolAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                lockTaken = await _initializationLock.WaitAsync(TimeSpan.FromSeconds(30), cancellationToken);
        ...
        private async Task<List<string>> FetchAllProxyListsAsync2(CancellationToken cancellationToken)
        {
            var allProxies = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            ...
            var fetchTasks = new List<Task<List<string>>>();

            foreach (string url in _proxyListUrls)
            {
                fetchTasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        ...
                        LogDebug($"Fetched {proxies.Count} proxies from {url}");
                        return proxies;
                    }
                    catch (Exception ex) when (!(ex is OperationCanceledException))
                    {
                        LogError($"Failed to fetch proxy list from {url}: {ex.Message}");
                        return new List<string>();
                    }
                }, cancellationToken));
            }
            ...
            try
            {
                // Wait for all tasks or until timeout/cancellation
                var completedTasks = await Task.WhenAll(fetchTasks);

                // Combine all proxy lists
                foreach (var proxyList in completedTasks)
                {
            ...
            {
                // Operation was canceled, return what we have so far
                LogWarning("Proxy list fetching was canceled");
            }
            catch (Exception ex)
            {
                LogError($"Error fetching proxy lists: {ex.Message}");
            }
        ...
        private async Task TestProxiesAsync(List<string> proxies, CancellationToken cancellationToken)
        {
            ...
            LogInfo($"Testing {shuffledProxies.Count} proxies...");

            // Create a completion source to track overall progress
            ...
            foreach (var address in shuffledProxies)
            {
                if (linkedCts.Token.IsCancellationRequested)
                    break;
                ...
                tasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        var proxyInfo = ParseProxy(address);
                        bool isWorking = false;
                        ...
                        if (isWorking)
                        {
                            ...
                            workingProxies.Add(proxyInfo);
                            LogInfo($"Found working proxy: {address}");

                            if (workingProxies.Count >= _minProxiesToFind)
                            {
                ...
                // Every 500 proxies, log progress
                if (tasks.Count % 500 == 0)
                {
                    LogInfo($"Tested {tasks.Count} proxies so far, found {workingProxies.Count} working ones");

                    // Clean up completed tasks to avoid memory issues
                    await Task.WhenAll(tasks.Where(t => t.IsCompleted).ToArray());
                    tasks.RemoveAll(t => t.IsCompleted);
                }

                // If we found enough working proxies or we're out of time, stop
                if (completionSource.Task.IsCompleted || timeoutCts.Token.IsCancellationRequested)
                {
                    break;
                }
            }
            ...
            LogInfo($"Proxy testing completed. Found {workingProxies.Count} working proxies out of {proxies.Count} tested");
        }
...
//        private readonly IEnumerable<string> _proxyListUrls;
//        private readonly int _testTimeoutSeconds;
//        private readonly int _fetchTimeoutSeconds;
//        private readonly int _maxParallelTests;
//        private readonly int _maxRetries;
//        private readonly bool _allowDirectFallback;
...
//            _testTimeoutSeconds = Math.Max(1, testTimeoutSeconds);
//            _fetchTimeoutSeconds = Math.Max(5, fetchTimeoutSeconds);
//            _maxParallelTests = Math.Max(1, Math.Min(50, maxParallelTests)); // Limit between 1 and 50
//            _maxRetries = Math.Max(1, maxRetries);
//            _healthCheckIntervalMinutes = Math.Max(5, healthCheckIntervalMinutes);
//            _allowDirectFallback = allowDirectFallback;
//            _userAgent = userAgent ?? "Mozilla/5.0";
...
//            catch (Exception ex)
//            {
//                LogError($"Unexpected error fetching {url}: {ex.MessageAnthropic}");
//                return string.Empty;
//            }
//        }
...
//                    proxyAddresses,
//                    cancellationToken,
//                    minProxiesToCheck: 50,  // Check at least 500 proxies
//                    maxTimeSeconds: 120       // Spend at most 60 seconds checking
//                );
...
//            foreach (string address in shuffledProxies)
//            {
//                // Stop if operation was canceled or we've timed out
//                if (linkedCts.Token.IsCancellationRequested || stopwatch.Elapsed.TotalSeconds > maxTimeSeconds)
//                {
...
//                // Stop if we've checked the minimum number of proxies AND have at least 5 working proxies
//                if (proxiesStartedChecking >= minProxiesToCheck && workingProxies.Count >= 5)
//                {
//                    LogInfo($"Found {workingProxies.Count} working proxies after checking {proxiesStartedChecking} proxies, stopping search");
//                    break;
//                }
...
//                            {
//                                // Log all exceptions and continue to next URL
//                                LogDebug($"Error testing proxy {address} with {testUrl}: {ex.MessageAnthropic}");
//                                continue;
//                            }
//                        }
...
//                    {
//                        // Catch ALL exceptions, log, and continue
//                        LogDebug($"Error processing proxy {address}: {ex.MessageAnthropic}");
//                    }
//                    finally
//                    {
//                        // Always release the semaphore
...
//                // Every 100 proxies, clean up completed tasks to avoid memory issues
//                if (proxiesStartedChecking % 100 == 0)
//                {
//                    // Wait for already completed tasks
//                    var completedTasks = tasks.Where(t => t.IsCompleted).ToArray();
//                    if (completedTasks.Length > 0)
...
//            {
//                proxy.RecordFailure();
//                LogDebug($"Error fetching {url} with proxy {proxy.Address}: {ex.MessageAnthropic}");
//                return string.Empty;
//            }
//        }
...
//                catch (Exception ex)
//                {
//                    LogDebug($"Error during attempt {attempt + 1} with proxy {proxy.Address} for {url}: {ex.MessageAnthropic}");
//                }
...
//                    catch (Exception ex) when (!(ex is OperationCanceledException))
//                    {
//                        LogError($"Failed to fetch proxy list from {url}: {ex.MessageAnthropic}");
//                        return new List<string>();
//                    }
//                }, cancellationToken));
//            }
...
//            catch (Exception ex)
//            {
//                LogError($"Error fetching proxy lists: {ex.MessageAnthropic}");
//            }
...
//            catch (Exception ex)
//            {
//                LogDebug($"Error parsing proxy address {proxyAddress}: {ex.MessageAnthropic}");
//                // Use defaults if parsing fails
//            }
...
//            catch (Exception ex) when (!(ex is OperationCanceledException))
//            {
//                LogDebug($"Error fetching {url} with direct connection: {ex.MessageAnthropic}");
//                return string.Empty;
//            }
//        }
...
//                            catch (Exception ex)
//                            {
//                                LogDebug($"Error during health check for {proxy.Address}: {ex.MessageAnthropic}");
//                                proxy.RecordFailure();
//                            }
//                            finally
//                            {
...
//                    catch (Exception ex)
//                    {
//                        LogError($"Error waiting for health check tasks: {ex.MessageAnthropic}");
//                    }
...
//                catch (Exception ex)
//                {
//                    LogError($"Error in proxy health check: {ex.MessageAnthropic}");
//                }
//            });
//        }
